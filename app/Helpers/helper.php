...
function handleFileUploads($request, $user)
{
    try
    {
        $fileFields = [
        ...
        foreach ($fileFields as $inputName => $column) {
            if ($request->file($inputName)) {
                // Assuming UploadFileManager is a custom class that handles file storage
...
function sluggablemethod($string, $separator = '-') {
