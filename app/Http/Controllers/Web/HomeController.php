...
class HomeController extends Controller
{
    ...
    {
        $filePath = public_path('downloads/teachmeapp.apk');

        if(!file_exists($filePath)) {
            abort(404, 'APK file not found.');
        }
        // Log entry content
        $logEntry = "[" . now() . "] APK downloaded from IP: " . request()->ip() . PHP_EOL;

        // Path to your custom log file
        $logFile = storage_path('logs/apk_downloads.log');

        // Append log entry
        file_put_contents($logFile, $logEntry, FILE_APPEND);

        // Serve the file download
      /*  return response()->download($filePath, 'teachmeapp.apk', [
            'Content-Type' => 'application/vnd.android.package-archive',
            'Content-Disposition' => 'attachment; filename="teachmeapp.apk"',
        ]);
*/
