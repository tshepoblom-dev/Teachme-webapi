...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function register(Request $request)
    {
        ...
        try{
           // UploadUserFiles::dispatch($request, $user);
            Bus::dispatch(function() use ($request, $user){
                handleFileUploads($request, $user);
            });
        }
        catch(Exception $ex)
        {
            \Log::error('Files upload failed: ' . $ex->getMessage());
        }
    ...
    public function searchCategories(string $request)
    {
        $categories = Category::where('slug', 'LIKE', "%{$request}%")->get();

        return response()->json($categories);
    }
    public function getSchools(string $request)
    {
        $schools = User::where('institution_name', 'LIKE', "%{$request}%")
                   ->distinct()
