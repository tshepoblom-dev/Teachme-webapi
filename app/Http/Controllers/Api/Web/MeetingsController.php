            ...
            if (!empty($meetingTime)) {
                ...
                if (!empty($meeting) and !$meeting->disabled) {
                    if (!empty($meeting->amount) and $meeting->amount > 0) {
                        ...
                        $rawMeetingTime = $request->input('meeting_time') ?? $meetingTime->time;
                        $explodetime = explode('-', $rawMeetingTime);
                        if (count($explodetime) !== 2) {
                            return apiResponse2(0, 'invalid_time', 'Invalid meeting time format.', null, trans('public.request_failed'));
                        }
