...
class ReserveMeetingController extends Controller
{
    ...
    public function finish(Request $request, $id)
    {
        ...
        if (!empty($ReserveMeeting)) {
            ...
            $startTime = strtotime($ReserveMeeting->start_at); // Meeting start time in timestamp
            $currentTime = time(); // Current time in timestamp
            $timeDifference = $startTime - $currentTime; // Difference in seconds
            $refundMessage = "Meeting finished with no refund.";

            if ($timeDifference > 1800) { // More than 30 minutes before start time (1800 seconds)
                    // Process refund
                    $refundAmount = $ReserveMeeting->paid_amount; // Assuming the paid_amount column stores the amount paid

                    if ($refundAmount > 0) {
                        // Add refund to user's wallet (assuming a wallet system exists)
                        $sale = Sale::where('id', $ReserveMeeting->sale_id)
                                ->whereNull('refund_at')
                                ->first();
                        if(!empty($sale))
                        {
                            ...
                            $refundMessage = "Meeting finished and refund issued.";
            ...
            // Delete related cart if it exists
            $cart = Cart::where('reserve_meeting_id', $ReserveMeeting->id)->first();
            if ($cart) {
                $cart->delete();
            }
    ...
    public function acceptMeetingRequest($id)
    {
        try{
            ...
            if (!empty($ReserveMeeting)) {
                ...
                // Check if the authenticated user is allowed to accept this meeting
                ...
                sendNotification('meeting_request_accepted', $notifyOptions, $ReserveMeeting->user_id);
