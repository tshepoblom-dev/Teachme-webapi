             ...
             {{--   <a href="@if($authUser->isUser()) /panel/webinars/purchases @else /panel/meetings/requests @endif" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center">
                ...
                <div class="bg-white account-balance rounded-sm panel-shadow py-15 py-md-30 px-10 px-md-20">
                    <div class="text-center">
                        ...
                        <h3 class="font-16 font-weight-500 text-gray">{{ $authUser->isUser() ? trans('panel.meetings') : trans('panel.monthly_sales') }}</h3>
                        <span class="mt-5 d-block font-30 text-secondary">{{ !empty($monthlySalesCount) ? handlePrice($monthlySalesCount) : (!empty($reserveMeetingsCount) ? $reserveMeetingsCount : 0) }}</span>
                    </div>
                    <div class="mt-20 pt-30 border-top border-gray300 d-flex align-items-center justify-content-between">
                        <a href="@if($authUser->isUser()) /panel/meetings/reservation @else /panel/financial/sales @endif" class="font-16 font-weight-bold text-dark-blue">
                          {{ $authUser->isUser() ? "View my meetings" : "View my sales"}}
                        </a>
